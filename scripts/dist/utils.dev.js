...
var utils = {
   ...
   */
  makeCouples: function makeCouples(tableNb) {
    var array = [];
   ...
   */
  createOperations: function createOperations(rounds, sign) {
    var operations = [];

    for (var roundIndex = rounds; roundIndex > 0; roundIndex--) {
      operations.push([sign, Math.floor(Math.random() * 98 + 2), Math.floor(Math.random() * 8 + 2)]);
    }
   ...
   * @returns 
   */
  prepareView: function prepareView(raceType, operationType) {
    var pageDetails = {
      startElt: null,
      comment: null,
      firstNumber: null,
    ...
    if (raceType == "sprint") {
      switch (operationType) {
        case "add":
          pageDetails.sign = '+';
          pageDetails.comment = 'On fait quelques additions ?';
          break;

        case "sub":
          pageDetails.sign = '-';
          pageDetails.comment = 'On fait quelques soustractions ?';
          break;
      ...
      pageDetails.comment = "Page pas reconnue ".concat(page);
    }

    return pageDetails;
  },
  // Fonction pour jouer une animation
  play: function play(elt, class_name) {
